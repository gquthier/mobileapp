...
CREATE INDEX idx_pending_jobs ON transcription_jobs(status, created_at) WHERE status IN ('pending', 'failed');
CREATE INDEX idx_user_jobs ON transcription_jobs(user_id, created_at DESC);
