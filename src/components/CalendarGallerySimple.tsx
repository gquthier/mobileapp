...
const CalendarGallerySimpleComponent: React.FC<CalendarGalleryProps> = ({
  ...
  // ✅ PHASE 2 OPTIMIZATION: Backend calendar data
  ...
  // ✅ Generate calendar data dynamically for all years with videos
  const calendarData = useMemo((): MonthData[] => {
    // ✅ PHASE 2: Use backend data if available (80% faster)
    if (useBackendData && backendCalendarData && backendCalendarData.length > 0) {
      console.log('📊 [PHASE 2] Using BACKEND calendar data (pre-calculated SQL)');

      // Transform backend data to MonthData[] format
      // Backend data is already grouped by day, we need to group by month
      ...
      backendCalendarData.forEach((dayEntry: any) => {
        ...
        // Reconstruct VideoRecord objects from backend data
        const dayVideos: VideoRecord[] = (dayEntry.video_ids || []).map((id: string, index: number) => ({
          id,
          title: dayEntry.video_titles?.[index] || '',
          thumbnail_path: dayEntry.thumbnail_paths?.[index] || null,
          thumbnail_frames: dayEntry.thumbnail_frames_array?.[index] || null,
          chapter_id: dayEntry.chapter_ids?.[index] || null,
          metadata: dayEntry.metadata_array?.[index] || {},
          transcription_status: dayEntry.transcription_statuses?.[index] || null,
          created_at: dayEntry.day,
        } as VideoRecord));
        ...
        monthData.days.set(day, {
          ...
          videos: dayVideos,
          isCurrentMonth: true,
          chapterColor: chapterColorsByDate[`${year}-${month}-${day}`],
        });

        monthData.videoCount += dayEntry.video_count;
      });

      // Convert to MonthData[] with full calendar grid
      const months: MonthData[] = [];
      monthsMap.forEach((monthData, monthKey) => {
        const { year, month, days, videoCount } = monthData;

        // Generate full calendar grid (35 cells = 5 weeks)
        ...
        const remainingDays = 35 - fullDays.length;
        for (let i = 1; i <= remainingDays; i++) {
          fullDays.push({
            date: i,
            dayOfWeek: fullDays.length % 7,
            videos: [],
            isCurrentMonth: false,
          });
        }

        // Reverse weeks (last week first)
        const reversedDays: DayData[] = [];
        for (let week = 4; week >= 0; week--) {
          const weekDays = fullDays.slice(week * 7, (week + 1) * 7);
          reversedDays.push(...weekDays);
        }
      ...
      console.log(`✅ [PHASE 2] Generated ${months.length} months from backend data`);
      return months.filter(m => m.videoCount > 0);
    }
    ...
    // Get all unique years from videos
    const videoYears = new Set<number>();
    Object.keys(videosByDate).forEach(dateKey => {
      const [year] = dateKey.split('-').map(Number);
      videoYears.add(year);
    });

    // If no videos, just show current year
    const yearsToShow = videoYears.size > 0
      ? Array.from(videoYears).sort((a, b) => b - a) // Newest first
      : [currentYear];

    // Generate months for all years with videos
    // Loop in reverse order (December to January) for each year
    yearsToShow.forEach(year => {
      ...
      // ✅ Inverse l'ordre des semaines (dernière semaine en haut, première en bas)
      const reversedDays: DayData[] = [];
      for (let week = 4; week >= 0; week--) {
      ...
      months.push({
        ...
        month,
        monthName: MONTHS[month],
        days: reversedDays, // ✅ Use reversed days (last week first)
        chapter: chapter?.title,
    ...
    // ✅ Already in correct order (year DESC, month DESC), just filter
    return months.filter(m => m.videoCount > 0); // ✅ Only show months with videos
  }, [useBackendData, backendCalendarData, videosByDate, chapterColorsByDate, chapters]);
