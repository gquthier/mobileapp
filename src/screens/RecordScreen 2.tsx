...
const RecordScreen: React.FC = () => {
  ...
  const transcribeVideo = async (videoUri: string): Promise<string> => {
    try {
      ...
      const preferredLanguage = currentUser?.profile?.preferred_language || 'auto';

      // Step 1: Extract audio from video
      console.log('📤 Extracting audio from video...');
      const audioPath = await AudioExtractionService.extractAudioForTranscription(videoUri);
      console.log('✅ Audio extracted:', audioPath);

      // Check if we're using the MOCK service (for Expo Go development)
      let isMockService = false;
      try {
        // Check if it's a mock file by reading its content
        const fileContent = await require('expo-file-system/legacy').readAsStringAsync(audioPath);
        isMockService = fileContent.includes('MOCK_AUDIO_DATA');
        console.log('🔍 File content check:', { audioPath, isMock: isMockService });
      } catch (error) {
        // If we can't read the file, assume it's a mock
        isMockService = audioPath.includes('cache/audio/') || audioPath.includes('extracted_audio_');
        console.log('🔍 Path-based detection:', { audioPath, isMock: isMockService });
      }

      let transcriptionResult;

      if (isMockService) {
        // Step 2a: Generate mock transcription for development
        console.log('🎭 Using mock transcription for Expo Go development');
        transcriptionResult = {
          task: 'transcribe' as const,
          language: preferredLanguage === 'auto' ? 'en' : preferredLanguage,
          duration: 15.5,
          text: "This is a mock transcription for development purposes. The video recording and playback features work correctly, but actual transcription requires a development build with native audio processing capabilities.",
          segments: [
            {
              id: '0',
              seek: 0,
        ...
        // Step 2b: Real transcription with OpenAI Whisper
        console.log('🔤 Transcribing audio with OpenAI...', { preferredLanguage });
      ...
      await AudioExtractionService.deleteTempFile(audioPath);
  ...
  const handleValidateVideo = async () => {
    ...
    try {
      ...
      // Show loading state
      Alert.alert('Processing...', 'Saving your video and generating transcription');

      // Step 1: Transcribe the video
      let transcriptionResult = null;
      let transcriptionText = '';
      try {
        transcriptionResult = await transcribeVideo(lastVideoUri);
        ...
        Alert.alert(
          ...
          'Your video will be saved but transcription could not be generated. You can try transcribing it later.',
          [{ text: 'Continue', style: 'default' }]
        );
      }

      // Step 2: Save video to Supabase with user authentication
      ...
      <View style={styles.content}>
        ...
        <View style={styles.cameraContainer}>
          ...
          >
            ...
            {showPostRecording && (
              <View style={styles.postRecordingOverlay}>
                {/* Video title input */}
