...
export class VideoService {
  ...
  private static async uploadWithProgression(
    ...
    const uploadResult = await FileSystemNew.uploadAsync(uploadUrl, videoUri, {
      httpMethod: 'POST',
      uploadType: FileSystemNew.FileSystemUploadType.MULTIPART,
      fieldName: 'file',
      headers: {
  ...
  static async uploadVideo(videoUri: string, title: string, userId?: string, chapterId?: string): Promise<VideoRecord | null> {
    ...
    try {
      ...
      // Step 3: Get video duration (needed for both local and remote records)
      let duration = 0;
      try {
      ...
      try {
        console.log('📸 Generating thumbnail frames locally...');
      ...
      // If upload failed, warn the user but still return the video record
      if (isLocalBackup) {
  ...
  static async getAllVideos(userId?: string): Promise<VideoRecord[]> {
    try {
      ...
      // Optimized: Single query with LEFT JOIN instead of N separate queries
  ...
  static async searchVideos(query: string, userId?: string): Promise<VideoRecord[]> {
    try {
      ...
      if (videosError) {
        console.error('❌ Error fetching videos for search:', videosError);
        return [];
      }
      ...
      // Get all transcription jobs for this user (from transcription_jobs table)
      const { data: transcriptionJobs, error: jobsError } = await supabase
        .from('transcription_jobs')
        .select('video_id, transcript_highlight, transcription_text')
        .eq('user_id', currentUserId);

      if (jobsError) {
        console.error('❌ Error fetching transcription jobs:', jobsError);
        // Continue without transcriptions - we can still search by title/date
      }

      // Create a map of video_id to transcription jobs
      const transcriptionMap = new Map();
      if (transcriptionJobs) {
        transcriptionJobs.forEach(job => {
          if (job.video_id) {
            if (!transcriptionMap.has(job.video_id)) {
              transcriptionMap.set(job.video_id, []);
            }
            transcriptionMap.get(job.video_id).push(job);
          }
        });
      }

      // Filter videos by title, date, and highlights
      const filteredVideos = videos.filter(video => {
        ...
        // Search in transcription text and highlights
        const videoJobs = transcriptionMap.get(video.id);
        if (videoJobs && Array.isArray(videoJobs)) {
          for (const job of videoJobs) {
            // Search in full transcription text
            if (job.transcription_text?.toLowerCase().includes(queryLower)) {
              return true;
            }

            // Search in highlights (transcript_highlight)
            if (job.transcript_highlight?.highlights && Array.isArray(job.transcript_highlight.highlights)) {
              const foundInHighlights = job.transcript_highlight.highlights.some((highlight: any) => {
                // Search in highlight title
                if (highlight.title?.toLowerCase().includes(queryLower)) {
                  return true;
                }
                // Search in highlight summary
                if (highlight.summary?.toLowerCase().includes(queryLower)) {
                  return true;
                }
                // Search in highlight themes
                if (highlight.themes && Array.isArray(highlight.themes)) {
                  return highlight.themes.some((theme: string) =>
                    theme.toLowerCase().includes(queryLower)
                  );
                }
                // Search in highlight category
                if (highlight.category?.toLowerCase().includes(queryLower)) {
                  return true;
                }
                return false;
              });

              if (foundInHighlights) {
                return true;
              }
            }
          }
